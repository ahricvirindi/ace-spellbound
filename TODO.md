...
## Admin Commands
- [ ] A /who command that lists who is online., their accout name, level, and location (resolve landblock to "name" (not all landblocks will be in the Town table...core ACE might have a table for locations, if not we need to build LandblockAliases as a thing).
...
- [ ] **Login-time safety net.** If `ApplyToCharacter` ever drops a phantom row (idempotency claim succeeds, mutation throws) the bonus is silently lost until manual recovery. Optional: on player login, walk `AccountAchievement` × `AwardedCharacterAchievements` for the logged-in character; for any granted achievement with no matching applied row, re-attempt the apply. One extra DB read per login is the cost; preventing silent loss of permanently-earned bonuses is the benefit.
- [ ] **Add a `LevelMin` filter variant** if "reach level N or higher (any future level)" becomes a real use case. Today `Level` matches exactly; per-level firing means "reach level 50" works as a single row, but compound rules ("on every level past 50") would need either repeated rows or a min-comparator.
